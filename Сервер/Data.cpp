...
{ mysql_init(&mysql);
    if(&mysql == nullptr)
    {std::cout << "Ошибка: дискритор соединения MySQL не создан." << std::endl;exit(1);}
    MYSQL *connect = mysql_real_connect(&mysql, "localhost", "root", "111", "testdb", 0, NULL, 0);
    if(connect == nullptr)
    {std::cout << "Невозможно подключится к базе данных. Ошибка:  " << mysql_error(&mysql) << std::endl;  exit(1);}    }
...
{const std::string str = "insert into users (id, name, login, user_password) values \
    ...
    if(mysql_errno(&mysql)) {return false;}
	else{ return true;}
}
...
{const std::string str = "select login, user_password from users where login='" + login +"' and \
    ...
    if(res)
    { if (mysql_fetch_row(res)){return true; }            
    else{ return false;}}
	else{ return false;}        }
...
{const std::string str = "SELECT login FROM users where login != '" + login + "'";
    ...
    {		    
        ...
        {for (auto i = 0; i < mysql_num_fields(res); i++) {tmp = tmp + row[i] +  " / ";}}
	}return tmp;}
...
{ const std::string str = "SELECT login FROM users ";
    ...
    if (res) 
    {while (row = mysql_fetch_row(res)) 
        {for (auto i = 0; i < mysql_num_fields(res); i++) {if(row[i] == login_to) return true;}}
        return false;}
	else { return false;}
}
...
{ const std::string str = "select login, s.text from users join \
    ...
    if (res = mysql_store_result(&mysql)) 
    {while (row = mysql_fetch_row(res)) 
        {for (auto i = 0; i < mysql_num_fields(res); i++) {tmp = tmp + row[i] +  "  / ";} tmp = tmp + "\n";}}
	     return tmp;}
...
void Data::closeData(){ mysql_close(&mysql);}    